...
constexpr int PROGRESS_UPDATE_INTERVAL = 250;  // in ms
...
inline std::atomic<int64_t> g_time_remaining{};		  // time remaining until completion in ms
...
inline void print_progress() {
	// get a snapshot of all changing atomics for consistency across this update
	int64_t bytesCompleted = g_bytes_completed.load();
	int64_t totalBytes = g_total_bytes.load();
	int64_t now = get_time();
	// get time and data sent
	double time_difference_sec = (now - g_speed_last_time.load()) / 1000.0;	 // time since last speed calc
	uint64_t data_difference = bytesCompleted - g_speed_last_bytes.load();

	if (totalBytes) {
		int percentage = ((double)bytesCompleted / totalBytes) * 100;
	...
	// calc speed
	if (data_difference) {
		uint64_t speed = data_difference / time_difference_sec;
		g_speed_bps.store(speed);
		g_time_remaining.store((totalBytes - bytesCompleted) / speed);
	}

	// update params for next calc
	g_speed_last_time.store(now);
	g_speed_last_bytes.store(bytesCompleted);
}